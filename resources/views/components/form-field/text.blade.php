<div class="form-group {{ $field->isInline ? 'form-inline' : '' }} {{ $field->css->classes }}" style="{{ $style }}">
    ...
    <div class="input-group">
        <input name="{{ $field->id }}" type="text" class="form-control" id="{{ $id }}" @if($field->placeholder) placeholder="{{ $field->placeholder }}" @endif value="{{ $field->default }}" @if($field->validation->isRequired) required="" @endif>
        @if($field->helper)
